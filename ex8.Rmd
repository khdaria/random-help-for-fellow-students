...
Perform a paired samples t-test on city and highway mileage (from the mpg data frame) twice:
one time using the jmv package, and one time using R's default t.test function.
...
A paired-samples t-test was conducted to compare means for city and highway fuel consumption. There was a significant difference in mileage for city (*M* = `r mean(mpg$cty, na.rm=T)`,*SD* = `r sd(mpg$cty, na.rm=T)`) and highway (*M* = `r mean(mpg$hwy, na.rm=T)`,*SD* = `r sd(mpg$hwy, na.rm=T)`); *t*(`r fit1$parameter`) = `r fit1$statistic`, `r frmtp(fit1$p.value, digit = 3, markup = "markdown", case = "lower")`, *Cohen's d* = `r fit1$effsize`.

**jamovi**

A paired-samples t-test was conducted to compare means for city and highway fuel consumption. There was a significant difference in mileage for city (*M* = `mean(mpg$cty,na.rm=T)`,*SD* = `sd(mpg$cty, na.rm=T)`) and highway (*M* = `mean(mpg$hwy, na.rm=T)`,*SD* = `sd(mpg$hwy, na.rm = T)`); *t*(`r fit2$df`) = `fit2$stat`, `frmtp(fit2$p, digit = 3, markup = "markdown", case = "lower")`, *Cohen's d* = `fit2$es`

A paired-samples t-test was conducted to compare means for city and highway fuel consumption. There was a significant difference in mileage for city (*M* = `r mean(mpg$cty,na.rm=T)`,*SD* = `r sd(mpg$cty, na.rm=T)`) and highway (*M* = `r mean(mpg$hwy, na.rm=T)`,*SD* = `r sd(mpg$hwy, na.rm = T)`); *t*(`r fit2$df`) = `r fit2$stat`, `r frmtp(fit2$p, digit = 3, markup = "markdown", case = "lower")`, *Cohen's d* = `r fit2$es`
...
Add confidence intervals (using the conf.low and conf.high columns) as error bars to the bar graph (read the help on geom_errorbar for more information).
...
An independent samples t-test was conducted to compare average wind speed at JFK and Newark airports. There was a significant difference in average wind speed at JFK (*M* = `r mean(filter(weather, origin == "JFK")$wind_speed, na.rm=T)`,*SD* = `r sd(filter(weather, origin == "JFK")$wind_speed, na.rm=T)`) and at Newark *M* = `r mean(filter(weather, origin == "EWR")$wind_speed, na.rm=T)`,*SD* = `r sd(filter(weather, origin == "EWR")$wind_speed, na.rm=T)`); *t*(`r fit$parameter`) = `r fit$statistic`, `r frmtp(fit$p.value, digit = 3, markup = "markdown", case = "lower")`, *Cohen's d* = `r fit$effsize`.
...
Different weather indicators explained a significant proportion of variance in arrival delay, $R^2$ = `summary(fit)$adj.r.squared`, *F*(`summary(fit)$fstatistic[2]`, `summary(fit)$fstatistic[3]`) = `summary(fit)$fstatistic[1]`, `frmtp(stats::anova(lm(arr_delay ~ ., data=to_lm))$'Pr(>F)'[1], digit = 3, markup = "markdown", case = "lower")`

Different weather indicators explained a significant proportion of variance in arrival delay, $R^2$ = `r summary(fit)$adj.r.squared`, *F*(`r summary(fit)$fstatistic[2]`, `r summary(fit)$fstatistic[3]`) = `r summary(fit)$fstatistic[1]`, `r frmtp(stats::anova(lm(arr_delay ~ ., data=to_lm))$'Pr(>F)'[1], digit = 3, markup = "markdown", case = "lower")`
...
```
Temperature, dewpoint, relative humidity, and all interactions among them explained a significant proportion of variance in arrival delay, $R^2$ = `summary(fit2)$adj.r.squared`, *F*(`summary(fit2)$fstatistic[2]`, `summary(fit2)$fstatistic[3]`) = `r summary(fit2)$fstatistic[1]`, `frmtp(stats::anova(lm(form, data=to_lm))$'Pr(>F)'[1], digit = 3, markup = "markdown", case = "lower")`. There is a significant interaction between temperature and dewpoint.

Temperature, dewpoint, relative humidity, and all interactions among them explained a significant proportion of variance in arrival delay, $R^2$ = `r summary(fit2)$adj.r.squared`, *F*(`r summary(fit2)$fstatistic[2]`, `r summary(fit2)$fstatistic[3]`) = `r summary(fit2)$fstatistic[1]`, `r frmtp(stats::anova(lm(form, data=to_lm))$'Pr(>F)'[1], digit = 3, markup = "markdown", case = "lower")`. There is a significant interaction between temperature and dewpoint.
...
```{r res = 300, fig.height = 10, fig.width = 10, message=F, warning=F}
...
A two-way ANOVA was run to examine the effect of origin and destination on arrival delay. The analysis revealed the significant effect of origin, *F*(`fit$df[fit$term == "origin"]`, `fit$df[fit$term == "Residuals"]`) = `fit$statistic[fit$term == "origin"]`, `frmtp(fit$p.value[fit$term == "origin"], digit = 3, markup = "markdown", case = "lower")`,  $\eta^{2}{p}$ = `fit$Partial.eta.2[fit$term == "origin"]`. Moreover, the significant effect for destination has also been found, *F*(`fit$df[fit$term == "dest"]`, `fit$df[fit$term == "Residuals"]`) = `fit$statistic[fit$term == "dest"]`, `frmtp(fit$p.value[fit$term == "dest"], digit = 3, markup = "markdown", case = "lower")`,  $\eta^{2}{p}$ = `fit$Partial.eta.2[fit$term == "dest"]`. Destination has the strongest effect on arrival delay
 
 

A two-way ANOVA was run to examine the effect of origin and destination on arrival delay. The analysis revealed the significant effect of origin, *F*(`r fit$df[fit$term == "origin"]`, `r fit$df[fit$term == "Residuals"]`) = `r fit$statistic[fit$term == "origin"]`, `r frmtp(fit$p.value[fit$term == "origin"], digit = 3, markup = "markdown", case = "lower")`,  $\eta^{2}{p}$ = `r fit$Partial.eta.2[fit$term == "origin"]`. Moreover, the significant effect for destination has also been found, *F*(`r fit$df[fit$term == "dest"]`, `r fit$df[fit$term == "Residuals"]`) = `r fit$statistic[fit$term == "dest"]`, `r frmtp(fit$p.value[fit$term == "dest"], digit = 3, markup = "markdown", case = "lower")`,  $\eta^{2}{p}$ = `r fit$Partial.eta.2[fit$term == "dest"]`. Destination has the strongest effect on arrival delay
...
Origins explained a significant proportion of variance in arrival delay, $R^2$ = `summary(fit)$adj.r.squared`, *F*(`summary(fit)$fstatistic[2]`, `summary(fit)$fstatistic[3]`) = `summary(fit)$fstatistic[1]`, `frmtp(stats::anova(lm(arr_delay ~ origin, data=flights))$'Pr(>F)'[1], digit = 3, markup = "markdown", case = "lower")`. Since Newark is a reference category, an average delay in this airport equal the intercept. It was found that delays in JFK are smaller ($\beta$ =  `summary(fit)$coefficients[2,1]`, `frmtp(summary(fit)$coefficients[2,4], digit = 3, markup = "markdown", case = "lower")`). The same trend was observed for LGA ($\beta$ = `summary(fit)$coefficients[3,1]`, `frmtp(summary(fit)$coefficients[3,4], digit = 3, markup = "markdown", case = "lower")`).


Origins explained a significant proportion of variance in arrival delay, $R^2$ = `r summary(fit)$adj.r.squared`, *F*(`r summary(fit)$fstatistic[2]`, `r summary(fit)$fstatistic[3]`) = `r summary(fit)$fstatistic[1]`, `r frmtp(stats::anova(lm(arr_delay ~ origin, data=flights))$'Pr(>F)'[1], digit = 3, markup = "markdown", case = "lower")`. Since Newark is a reference category, an average delay in this airport equal the intercept. It was found that delays in JFK are smaller ($\beta$ =  `r summary(fit)$coefficients[2,1]`, `r frmtp(summary(fit)$coefficients[2,4], digit = 3, markup = "markdown", case = "lower")`). The same trend was observed for LGA ($\beta$ = `r summary(fit)$coefficients[3,1]`, `r frmtp(summary(fit)$coefficients[3,4], digit = 3, markup = "markdown", case = "lower")`).
